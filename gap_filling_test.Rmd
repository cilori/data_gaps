...
time_series_comparison <- function(num_years) {
  variable <- "CHL_POLY4"
  region <- "NWA"
  input_years <- ifelse(num_years==0, year, paste0(range(plus_minus(year,num_years)), collapse="-"))
  input_name <- paste0("output/filled_", region, "_", sensor, "_", variable, "_", input_years, "_")
...
tsc <- time_series_comparison(0)
attr_df <- tsc$attr_df
week_rmses <- tsc$week_rmses

stats_to_plot <- cbind(rep(1,4),
...
num_years <- 1
tsc <- time_series_comparison(num_years)

attr_df <- tsc$attr_df
week_rmses <- tsc$week_rmses

stats_to_plot <- rbind(stats_to_plot,
                       cbind(rep(num_years*2+1,4),
                             c("EOF", "RMSE", paste0("Week",weeks,"_RMSE"), "Processing_time"),
                             c(attr_df$eof, attr_df$rmse, week_rmses, attr_df$time_to_process)))
